...
export const StatusBadge = (props: Props) => {
  ...
  const shouldAnimate = () =>
    ['deploying', 'restarting', 'starting', 'pending'].includes(props.status);
  ...
  return (
    ...
    >
      <Show when={props.showDot !== false}>
        <span
          class={`relative flex h-1.5 w-1.5 ${shouldAnimate() ? 'animate-pulse' : ''}`}
        >
          <span
...
export const StatusDot = (props: { status: Status | string }) => {
  ...
  return (
    <span
      class={`relative flex h-2 w-2 ${shouldAnimate() ? 'animate-pulse' : ''}`}
    >
      <span
