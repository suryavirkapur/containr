...
import { getContainerStatus } from "../api/containers";
import {
	getService,
...
import { EmptyBlock, KeyValueTable, LoadingBlock, Notice, Panel } from "../components/Plain";
...
const displayContainerStatus = (status: string, restartCount: number): string => {
...
const MaskedValue = (props: { value: string }) => {
	...
	return (
		<span class="inline-flex items-center gap-2 font-mono text-sm">
			<span class={shown() ? "" : "blur-sm select-none pointer-events-none"}>{props.value}</span>
			<button
				type="button"
...
const ServiceDetail = () => {
	...
	const liveServiceStatus = createMemo(() => {
		...
		if (statuses.length > 0 && statuses.every((status) => status.status === "exited")) {
			return "stopped";
		}
	...
	createEffect(() => {
		...
		if ((service()?.container_ids ?? []).length === 0) return;
		const interval = setInterval(() => {
			void refetchContainerStatuses();
		}, 5000);
		onCleanup(() => clearInterval(interval));
	});
	...
	const updateEnvVar = (
		...
		setEnvVars((prev) =>
			prev.map((e, i) => (i === index ? { ...e, [field]: value, isEdited: true } : e)),
		);
	};
	...
	return (
		<div class="flex flex-col gap-6">
			{/* Back link */}
			<A href="/services" class="text-xs text-muted-foreground hover:text-foreground w-fit">
				← Services
			</A>
						...
						<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
							<div class="flex flex-col gap-1">
								<div class="flex items-center gap-2">
									<h1 class="text-2xl font-semibold tracking-tight">{currentService().name}</h1>
									<StatusBadge status={liveServiceStatus()} />
								</div>
								<p class="text-sm text-muted-foreground">
									{currentService().service_type.replace(/_/g, " ")}
									{currentService().project_name ? ` · ${currentService().project_name}` : ""}
								</p>
							</div>
						...
						<Tabs tabs={tabs()} activeTab={activeTab()} onTabChange={setActiveTab}>
							<Switch>
									...
									<div class="flex flex-col gap-4">
										...
										<Panel title="Project">
											<div class="flex flex-col gap-4">
												<div class="flex items-center gap-3">
													<span class="text-sm text-muted-foreground">Current project:</span>
												...
												<div class="flex flex-col gap-2">
													...
													<div class="flex gap-2">
														<select
															...
															value={targetGroupId() ?? ""}
															onChange={(e) => setTargetGroupId(e.currentTarget.value || null)}
														>
															<option value="">— keep current —</option>
															<option value="__none__">Remove from project (isolate)</option>
													...
													<p class="text-xs text-muted-foreground">
														Services in the same project share an internal Docker network.
													</p>
												</div>
											</div>
										</Panel>
												...
												>
													...
													>
														...
														<div class="flex flex-col gap-3">
															...
															<p class="text-xs text-muted-foreground">
																Point DNS A record to this server's IP or CNAME the default URL.
															</p>
														</div>
										...
										<Show when={!isAppService() && currentService()}>
											<Panel title="Connection">
												<KeyValueTable
													rows={[
														["Endpoint", <MaskedValue value={endpointFor(currentService())} />],
														["Internal host", currentService().internal_host ?? "—"],
											...
											<div class="flex flex-col gap-4">
												<Panel title="Scaling">
													<div class="flex flex-col gap-3 max-w-xs">
														...
														<p class="text-xs text-muted-foreground">
															Running: {currentService().running_instances} / {replicas()} desired
														</p>
													</div>
												</Panel>
										...
										{(currentSettings) => (
											<div class="flex flex-col gap-4">
												<Panel title="Deploy">
													<div class="flex flex-col gap-4">
														<div class="grid gap-4 sm:grid-cols-2">
															...
															<label class="cr-field">
																<span class="cr-label">Commit SHA (optional)</span>
																<input
																	class="cr-input font-mono"
												...
												<Panel title="Port Info">
													<KeyValueTable
														rows={[
															["Container Port", String(currentSettings().service.port ?? "—")],
															[
																"HTTP Exposed",
																currentSettings().service.expose_http ? "Yes" : "No",
															],
															[
																"Additional Ports",
																currentSettings().service.additional_ports.join(", ") || "None",
															],
															["Public Port", String(currentService().external_port ?? "None")],
														]}
													/>
												</Panel>
								...
								<Match when={activeTab() === "logs"}>
									<div class="flex flex-col gap-4">
										<Show when={(containerStatuses() ?? []).length > 0}>
											<Panel title="Runtime" subtitle="Live Docker container state">
												<div class="overflow-x-auto">
													<table class="cr-table">
														...
														<tbody>
															<For each={containerStatuses() ?? []}>
																{(container) => (
																	<tr>
																		<td class="font-mono text-xs">{container.id}</td>
																		<td>
																			<StatusBadge
																				status={displayContainerStatus(
																					container.status,
																		...
																		<td>{formatDateTime(container.finished_at)}</td>
																		<td>
																			{container.health_status ? (
																				<span class="text-xs">{container.health_status}</span>
																			) : (
												...
												<Show
													when={(containerStatuses() ?? []).some(
														(container) => container.status === "restarting",
													)}
												>
													<p class="mt-3 text-sm text-yellow-300">
														Docker reports this service is restarting. That usually means the
														container process is exiting immediately and Docker keeps re-launching
														it because the restart policy is enabled.
													</p>
												</Show>
											</Panel>
										</Show>
