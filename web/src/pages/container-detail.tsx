...
const ContainerDetail = () => {
	...
	const [socketState, setSocketState] = createSignal<"idle" | "connecting" | "open" | "closed">(
		"idle",
	);
	let socket: WebSocket | null = null;

	createEffect(() => {
		void store.loadContainers();
	});
	...
	const [files, { refetch: refetchFiles }] = createResource(
		...
		(value) =>
			value
				? listContainerFiles(value.id, { mount: value.mount, path: value.path })
				: Promise.resolve([]),
	);

	const currentContainer = createMemo(
		() => store.state.containers.find((container) => container.id === containerId()) ?? null,
	);
	...
	return (
		<div class="flex flex-col gap-8">
			...
			<Show when={status.error}>
				{(error) => (
					<Notice tone="error">Failed to load container status: {describeError(error())}</Notice>
				)}
			</Show>
			...
			<Panel title="Summary">
				<KeyValueTable
					rows={[
						...
						[
							"Resource Type",
							<span class="capitalize">{currentContainer()?.resource_type ?? "Unknown"}</span>,
						],
						[
							"Resource ID",
							<span class="font-mono text-muted-foreground">
								{currentContainer()?.resource_id ?? "Unknown"}
							</span>,
						],
						[
							"Status",
							<span
								class={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wider ${
									status()?.status === "running"
										...
										: status()?.status === "exited" || status()?.status === "dead"
						...
						[
							"Health",
							<span
								class={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wider ${
			...
			<Panel
				title="Shell Access"
				subtitle="Requests an exec token and opens a browser shell session."
			>
				<div class="flex flex-col gap-6">
					...
					<div class="flex flex-col border rounded-lg overflow-hidden border-border">
						<pre class="bg-[#111] text-[#fafafa] p-4 overflow-x-auto text-sm font-mono min-h-[16rem] whitespace-pre-wrap">
							{shellOutput() || "Shell output will appear here..."}
						</pre>
			...
			<Panel title="Container Logs">
				...
				<pre class="bg-card border border-border rounded-lg p-4 overflow-x-auto text-sm font-mono text-muted-foreground min-h-[14rem]">
					{logs() ?? ""}
				</pre>
			</Panel>
				...
				<Show
					...
					fallback={
						<p class="text-sm text-muted-foreground p-4 text-center border rounded-lg border-dashed">
							No writable or mounted volumes detected.
						</p>
					}
				>
					...
					<Show when={files.error}>
						{(error) => (
							<Notice tone="error">Failed to load files: {describeError(error())}</Notice>
						)}
					</Show>
								...
								{(entry) => (
									<article class="rounded-xl border bg-card text-card-foreground shadow-sm p-4 flex flex-col gap-4">
										<div class="flex justify-between items-start gap-4">
											...
											<span
												class={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wider ${
													entry.is_dir
														? "bg-secondary text-secondary-foreground border-border"
														: "bg-muted text-muted-foreground border-border"
												}`}
...
const EmptyState = (props: { path: string }) => (
	<div class="rounded-xl border border-dashed border-border bg-card p-8 text-center">
		...
		<p class="text-sm text-muted-foreground mt-2">
			{props.path ? `The directory ${props.path} is empty.` : "The selected mount root is empty."}
		</p>
	</div>
);
