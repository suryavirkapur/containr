...
fn parse_app_id_from_container(container_id: &str) -> Option<Uuid> {
    if !container_id.starts_with("containr-") {
        return None;
    }
    let suffix = &container_id["containr-".len()..];
    if suffix.len() < 36 {
        return None;
    }
...
fn resolve_exec_command(
    ...
    let selected = shell.unwrap_or("/bin/sh").trim();
    let command = match selected {
        "/bin/sh" | "sh" => vec!["/bin/sh".to_string()],
        "/bin/bash" | "bash" => vec!["/bin/bash".to_string()],
        "/bin/ash" | "ash" => vec!["/bin/ash".to_string()],
        _ => {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(ErrorResponse {
                    error: "unsupported shell".to_string(),
                }),
            ));
        }
    };
...
async fn handle_container_exec(
    ...
    let mut session = match docker
        .start_exec_session(&container_id, command, cols, rows)
        .await
    {
        Ok(session) => session,
        Err(error) => {
        ...
        tokio::select! {
            ws_message = socket.recv() => {
                match ws_message {
                    Some(Ok(Message::Binary(data))) => {
                        ...
                        if session.input.flush().await.is_err() {
                            break;
                        }
                    }
                    Some(Ok(Message::Text(text))) => {
                        match serde_json::from_str::<ExecControlMessage>(&text) {
                            Ok(ExecControlMessage::Resize { cols, rows }) => {
                                if docker.resize_exec(&session.exec_id, cols, rows).await.is_err() {
                                    break;
                                }
                            }
                            Err(_) => {}
            ...
            output = session.output.next() => {
                match output {
                    ...
                    None => {
                        let _ = socket
                            .send(Message::Text("[exec session ended]".into()))
                            .await;
                        break;
                    }
                }
...
mod tests {
    ...
    fn resolve_exec_command_accepts_known_shells() {
        assert_eq!(
            resolve_exec_command(Some("/bin/sh")).unwrap(),
            vec!["/bin/sh".to_string()]
        );
        assert_eq!(
            resolve_exec_command(Some("bash")).unwrap(),
            vec!["/bin/bash".to_string()]
        );
        assert_eq!(
            resolve_exec_command(Some("ash")).unwrap(),
            vec!["/bin/ash".to_string()]
        );
    }

    #[test]
    fn resolve_exec_command_rejects_unknown_shells() {
        match resolve_exec_command(Some("/usr/bin/python")) {
            Ok(_) => panic!("expected unknown shell to be rejected"),
            Err((status, body)) => {
