...
impl ImageManager {
    ...
    pub async fn pull_image(&self, name: &str) -> Result<ImageInfo> {
        self.pull_image_with_credentials(name, None).await
    }

    /// pulls an image from a registry with optional credentials
...
mod tests {
    ...
    fn resolve_registry_server_defaults_to_docker_hub() {
        assert_eq!(
            resolve_registry_server("redis:7", None),
            "https://index.docker.io/v1/"
        );
    }

    #[test]
    fn resolve_registry_server_uses_registry_prefix() {
        assert_eq!(
            resolve_registry_server("ghcr.io/demo/private:1", None),
            "ghcr.io"
        );
    }
