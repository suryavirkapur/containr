...
4. key features to build/fix next
-----------------------------------
- ui rewrite: remove the heavy "project" scaffolding from the solid.js frontend. flatten the hierarchy into an intuitive service list.
- service grouping form: during creation or editing, a user just picks a "network group" from a dropdown.
- network routing: ensure pingora proxies requests perfectly to single services without requiring a "primary project domain". domains belong to the service.
